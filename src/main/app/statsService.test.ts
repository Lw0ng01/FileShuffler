...
function setup(): { db: IndexDb; service: StatsService; views: StatsView[] } {
  ...
  const service = new StatsService({ db: localIndexStore(db) })
...
describe('StatsService', () => {
  it('starts with nothing played', async () => {
    const { service } = setup()
  ...
  it('summarises play history alongside the index', async () => {
  ...
  it('stars a file from the index or from play history', async () => {
    const { db, service } = setup()
    db.putFiles(db.startScan(), [indexed('D:\\Media\\indexed.mp4')])
    db.recordOpened('E:\\Shows\\played.mkv', 'played.mkv', 'E:\\Shows', 1)

    await service.addFavorite('D:\\Media\\indexed.mp4')
    const view = await service.addFavorite('E:\\Shows\\played.mkv')

    expect(view.favorites.map((row) => row.name).sort()).toEqual(['indexed.mp4', 'played.mkv'])
    expect(view.lastError).toBeNull()
  })

  it('refuses to star a path the app has never seen', async () => {
    const { service } = setup()
    const view = await service.addFavorite('C:\\Windows\\notepad.exe')
  ...
  it('removes a favorite', async () => {
    ...
    await service.addFavorite('D:\\Media\\a.mp4')

    expect((await service.removeFavorite('D:\\Media\\a.mp4')).favorites).toEqual([])
  })

  it('lists more never-played videos on request, in the chosen order', async () => {
    const { db, service } = setup()
    ...
    expect((await service.neverPlayed(50, 'size')).map((row) => row.name)).toEqual([
      'big.mp4',
      'small.mp4'
    ])
    expect((await service.neverPlayed(1, 'modified')).map((row) => row.name)).toEqual(['small.mp4'])
  })

  it('tells the window when something played', async () => {
    const { db, service, views } = setup()
    db.recordOpened('D:\\Media\\a.mp4', 'a.mp4', 'D:\\Media', 1)
    await service.notifyChanged()
